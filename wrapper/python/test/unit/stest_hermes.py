...
from py_hermes import Hermes, TRANSPARENT_HERMES, Bucket, Context
...
class AccessInfo:
    ...
    def __init__(self, access_info):
        ...
        self.tag_id_real_ = access_info.tag_id_
...
class AccessPatternLog:
    ...
    def reorganize(tag_id_real, blob_id_real, score):
        ...
        ctx = Context()
        bkt.reorganize_blob(blob_id_real, score, ctx)
...
def test_access_log():
    ...
    if len(mdm.stats) > 0:
        entry = mdm.stats[0]
        AccessPatternLog.reorganize(entry.tag_id_real_, entry.blob_id_real_, 0)
